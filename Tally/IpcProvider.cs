...
public sealed class IpcProvider : IDisposable
{
    ...
    /// <summary>
    /// The same feed the kill gate sends, raised in process for the train
    /// watcher, which used to receive it by subscribing to that gate from the
    /// other side of Dalamud's IPC.
    ///
    /// Deliberately raised from Send rather than from the tracker's events, so
    /// the train sees exactly what an external subscriber sees - including the
    /// PublishAllMarkDeaths switch between the credited and all-deaths feeds,
    /// and its guarantee that a death produces exactly one message either way.
    /// </summary>
    public event Action<KillDetail>? KillPublished;
    ...
    {
        // Before the gate, and not conditional on it: the train watcher lives
        // in this assembly now and must still be fed when IPC registration
        // failed (a stale copy of the standalone plugin holding the gate name).
        try
        {
            KillPublished?.Invoke(kill);
        }
        catch (Exception ex)
        {
            Service.Log.Error(ex, $"An in-process kill subscriber threw while handling {kill.Mark.Name}.");
        }
