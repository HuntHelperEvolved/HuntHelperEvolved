...
public sealed class LifestreamTravel : IDisposable
{
    ...
    private readonly ICallGateSubscriber<uint, bool> _changeWorld;
    private readonly ICallGateSubscriber<int> _currentInstance;
    private readonly ICallGateSubscriber<int> _instanceCount;
    ...
    {
        _framework=framework; _detector=detector; _chat=chat; _log=log;
        _isBusy = plugin.GetIpcSubscriber<bool>("Lifestream.IsBusy");
        _changeWorld = plugin.GetIpcSubscriber<uint, bool>("Lifestream.ChangeWorldById");
        _currentInstance = plugin.GetIpcSubscriber<int>("Lifestream.GetCurrentInstance");
        _instanceCount = plugin.GetIpcSubscriber<int>("Lifestream.GetNumberOfInstances");
        _canChangeInstance = plugin.GetIpcSubscriber<bool>("Lifestream.CanChangeInstance");
        _changeInstance = plugin.GetIpcSubscriber<int, object>("Lifestream.ChangeInstance");
        _teleport = plugin.GetIpcSubscriber<uint, byte, bool>("Lifestream.Teleport");
        _abort = plugin.GetIpcSubscriber<object>("Lifestream.Abort");
        framework.Update += Update;
    }
    public bool Available => _isBusy.HasFunction;
    ...
    public void Start(uint world, uint territory, Vector2 position, uint instance = 0)
    {
        try
        {
            ...
            if (_detector.CurrentWorldId() != world && !_changeWorld.InvokeFunc(world))
        ...
        try
        {
            ...
            if (_instanceHandoff is { } instanceHandoff)
            {
                ...
                var action = instanceHandoff.Step(now, _detector.CurrentWorldId(), _detector.CurrentTerritoryId,
                    atDestination ? (uint)Math.Max(0, _currentInstance.InvokeFunc()) : 0,
                    atDestination ? _instanceCount.InvokeFunc() : 0,
                    atDestination && _canChangeInstance.InvokeFunc(), busy, ready);
                switch (action)
                {
                    case InstanceTravelAction.Change:
                        _changeInstance.InvokeAction((int)_destination.Instance);
    ...
    public void Cancel()
    {
        ...
        _teleportWasActive=false;
        try { _abort.InvokeAction(); Status="Travel cancelled."; }
        catch (Exception ex) { Fail(ex); }
    }
