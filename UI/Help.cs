...
public sealed partial class Plugin
{
    ...
    [
        ...
        ("Getting around",
            "Train and the /hht popout share Route, Reports and Setup pages, with a pause/resume icon at the right of the page tabs. Next Mark and Next Aetheryte appear below the tabs only on Route, above compact, single-line mark rows with local/shared and preset context. Reports reviews scouting or completion reports, and Setup holds presets, clipboard tools, credits, watches and recovery. Changing page does not change the train or recording state. S Ranks contains timers, mapping and spawn counters. Settings groups preferences by task; Help explains the controls. The Windows menu opens standalone boards and popouts. The plugin installer lists the /hh command family when Hunt Helper is absent, or the /htr family when installed (including disabled copies). Extra aliases remain usable when not held by another plugin. Changes save as you edit.\n\n" +
        ...
        ("Train controls, scouting and reports",
            "Scouting starts paused after login. Resume to join scout credits and pick up new marks. Pause stops new records; existing health and death reports still update. Next Mark selects, flags and announces the next live mark without recording a kill. Next Aetheryte announces and copies its destination without teleporting or moving the flag. TP sits to the left of each mark's name; travelling to a rally stop completes it after a short delay. Click a mark's text to flag it; chat echo is optional. The check, crosshairs and X buttons sit on the right. The check records a witnessed kill now or restores a dead mark. Crosshairs records found gone without inventing a kill time; clearing that evidence leaves the mark dead with an unknown time. Hold Ctrl and click X to remove the row. Setup > Route tools and view includes Show spicing markers, the same preference as Settings > Train. When enabled, right-click the mark row and choose Being spiced.\n\n" +
            "World headings keep marks from different worlds separate. While scanning is active, Route offers a preset dropdown, followed by a separate row with Add Flag and an optional name box. Add Flag adds your current map flag as a custom stop. Choosing or reselecting a preset resumes its ordering and rally stops. Setup contains preset management, Group expansions within worlds and Open next automatically, plus import/export and the same Add Flag controls. The optional name draft is shared between both pages. Drag rows within their world/expansion or drag a heading to reorder expansions within one world. Recorded totals exclude rally stops and remain stable when dead rows are hidden. Hide dead changes the display; Remove Dead removes rows but retains report history. Imports merge and export codes retain worlds, instances and custom stops.\n\n" +
            "Setup also manages scout credits. Type a manual name, then press Enter or Add scout to share it; unsubmitted name drafts stay local. Hold Shift and click End train at the bottom of Route to send the completion report. Completing the last leg clears credits and removed-name suppression and pauses scouting; partial reports keep the remaining train's credits. Reports offers Scouting report and Train completion, with the relevant Shift-click send button below the preview. Webhook progress and results appear only on Reports in either train view. Conflicting train edits are disabled while posting; Next Mark and Next Aetheryte remain usable on Route.\n\n" +
            "Train completion clears reported dead marks only after Discord success and, for shared trains, server acknowledgement. Unfinished legs remain. Failed submission or concurrent edits retain the train. If acknowledgement is lost, inspect state before posting again. Setup > Recovery contains Shift-click Reset train, which posts nothing, and Undo reset. Undo restores locally and turns train sharing off without replacing your friends' train; a recovery notice also appears after a reset."),
        ("Scout notes and report preview",
            "Reports > Scouting report shows an optional note of up to 256 Unicode code points, credited scouts and the outgoing content. The draft is shared between the main Train tab and popout, stays local until sent, and lasts for the current plugin session. Closing a window or changing page keeps it. A successful send clears only the submitted draft; failures or partial delivery retain it, and edits made during sending are kept for the next report.\n\n" +
            "A reset or replacement train, including a shared snapshot on joining or reconnecting, detaches an unsent note so it is not automatically included in the new report. Previous note lets you explicitly reuse or discard it. Local Undo reset can restore the draft when no newer edit would be overwritten.\n\n" +
            "Scouting reports group last recorded state by world and expansion, with compact remaining/expected counts such as 1/12 for a partial scout. Totals use the full expansion roster and known instances per zone from server metadata or scout records; three instances across all six zones give 36 expected marks. Marked snipes, witnessed kills and unknown deaths remain distinct; Not recorded means absent from the retained list, not proof a mark is absent in an instance. The intact import code comes first, followed by the summary, notes and scout credits in one card within one Discord message when they fit. Otherwise, the code is sent first and the report second. The preview shows the count; if either part exceeds its own limit, nothing is sent and the draft is kept. Sending refreshes the current train snapshot. While posting, the preview shows the captured report and new note edits wait for the next one. Completion reports retain their separate respawn-window summaries and individual history."),
        ("Automatic train watches",
            "Train > Setup > S-rank watch setup can automatically follow open timer windows for Tyger, Ophioneus, Narrow-rift and Neyoozoteel in the train's worlds, zones and instances. Enable this on the client preparing the train. Manual watches and recorded check results are preserved; unchecked automatic watches are removed when no longer eligible. Turn automation off before removing an automatic watch yourself. Shared automation needs an updated server. Watch results remain available under the route while conducting.\n\n" +
        ...
        ("Map overlays and S-rank mapping",
            "Spawn points show possible locations; live dots show marks currently seen by you or the group. Rank filters for points and live marks are independent. Hold Alt and left-click a spawn point to flag it when enabled; ordinary clicks do not flag it. SS event locations use the same shortcut. Ctrl still hides the overlay, and Shift-click still toggles manual S-rank exclusions on supported points. Names and HP follow live-mark filters. Remote observations expire after three seconds without updates.\n\n" +
        ...
        ("Discord setup",
            "Create a webhook in Discord through Channel Settings > Integrations > Webhooks, then add its URL in Settings > Discord. Disabled entries remain saved but receive no posts. Send test message posts to enabled entries. Train > Reports previews the selected report and shows the message count per enabled destination. Scout reports put the import code first and fit into one message when possible, or two messages with the code first and the report second. Completed-train reports summarize respawn ranges and retain sniped, missing and unknown-time details. If some messages or destinations succeed and others fail, inspect Discord before retrying to avoid duplicate posts."),
        ("Trigger counters and lifetime tally",
            "S Ranks > Counters contains trigger-mob totals, world selection and current-zone spawn watches, including Narrow-rift and Nunyunuwi. Settings > S Ranks controls kill counting. Train > Setup > S-rank watch setup manages train checks. Trigger counters measure progress for a spawn attempt. The group total appears in brackets after your total. Shared counts use personal contributions so nearby observers do not double-count. Local Reset affects only you; Reset shared starts a new group attempt and discards pending contributions for the old attempt.\n\n" +
