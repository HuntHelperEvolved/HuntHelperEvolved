...
public sealed partial class Plugin
{
    ...
    [
        ("Getting around",
            "Conductor manages a train and its reports; Train shows the route and import/export controls; Scout prepares a scouting report; Marks Slain shows report history. Settings groups preferences by task. Changes save as you edit.\n\n" +
            "Commands: /hh opens the main window; /hht the train; /hhc the trigger counters; /hhs the S-rank timers; /hha the A-rank timers; /hhv or /hhsa Active Marks; /hunttally the lifetime tally. /hhn flags the next live train mark; /hhna names its nearest aetheryte; /htrm toggles map controls."),
        ("Train controls, scouting and reports",
            "Pause stops picking up new marks. Next advances to a live mark without recording a kill. Add flag inserts your map flag as a custom stop. Click a mark to flag it; the chat echo is optional. Spicing marks a scout's plan to prepare a mark before the train arrives. Hide dead affects only the display, not reports.\n\n" +
            "Group by expansion keeps marks together and opens the next unfinished group. Automatic scout names combine with manual credits. Collapse Train controls & scouts to keep the popout compact. Export codes carry the train's worlds, instances and custom stops.\n\n" +
